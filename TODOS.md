...
## [P1] moneyball CI fix — ws/vite 취약점 (2026-06-16, 반복 누적 42건)

**What**: moneyball `pnpm audit --audit-level=high` 실패. ws <8.21.0 (Memory exhaustion DoS) + vite <=8.0.15 (server.fs.deny bypass).
**Why**: CI 전체 블로킹. 허브 batch-close 누적: 28건(1110) + 5건(1112-1114) + 6건(1115-1117) + 3건(1124) = **42건 총 close**. playbook 자체는 cycle 1112 fix 완료.
**Action (사용자 영역)**: moneyball 레포에서 `pnpm update ws vite` → CI 재확인 → push. 마지막 이슈: 2026-06-17 23:48 KST.
...
## [DONE 2026-05-28 cycle 1044] R6 BRANCHED steady-state 구조 인정 — TODOS 항목 종료

**상태**: cycle 977 ~ 1044 동안 추적 미갱신 (67 cycle stale). cycle 1044 mid-review 시 현 BRANCHED = 0/1 (clean except 1 retro commit) 측정. 4번째 user-initiated merge 들 (cycle 1043 본 세션 rebase + push) 로 자연 해소 반복.

**최종 결론**: BRANCHED 재발 = 워커 auto-ingest (4-6/일) + 허브 cycle commits (1-2/cycle) 양방향 발산 구조의 **자연 결과**. PENDING 항목 아님. 사용자 batch 머지/rebase 시점에 자연 해소 → 즉시 재발 → 다음 batch — 4 cycle period 발견.

**누적 머지 이력 (4건)**:
- cycle 913 `9c1589d` (112건 흡수, +6 entries)
- cycle 943 `5904827` (23건 흡수, +4 entries)
- cycle 973 `39d3a30` (worker auto-ingest batch)
- cycle 1043 본 세션 rebase + push (auto-ingest 3건 + cycle 1043/1044 retro 합산)

**누적 워커 lesson 흡수**: 135+건 (cycle 913 + 943 기준, 이후 4-6/일 자연 적층).

**메모리 갱신**: memory `project_bidirectional_compound_growth_goal` 의 "BRANCHED local 11/origin 0" stale 박제 → 정확한 값 "steady-state 4 cycle period, 매 batch 자연 해소" 로 갱신 권장.

**stale 이유 (carry-over #C 분석)**:
- 매 cycle 측정 + TODOS append → cycle-by-cycle 200+ 항목 누적 → 자가 무덤 패턴
- 측정 자체는 evidence 가치 있지만 TODOS 가 보존 layer 부적합 (git log + memory entry 가 정합)
- 룰 갱신: BRANCHED 추적은 weekly mid-review 시 1회 측정만 + memory 갱신

**다음 액션**: 본 항목 DONE 종료. BRANCHED 측정은 mid-review cycle 자연 발화.
