...
tags:
  ...
  - evaluation
confidence: 1
connections: [harness-engineering/drift-detection-methodology, harness-engineering/guard-test-pattern, harness-engineering/question-own-defaults, journal/playbook-journal-003-drift-silent-bugs, prompt-engineering/prompt-versioning-and-testing, journal/playbook-journal-005-moneyballscore-playbook-e2e, context-engineering/ambient-knowledge-injection, prompt-engineering/llm-prompt-optimization-role-definition-constraint-design, harness-engineering/eslint-markdown-mdx-content-linting-strategy, harness-engineering/harness-engineeringresponsible-vibe-coding-with-claude-harne, agents/multi-agent-system-design-patterns]
status: draft
...
1.  LLM 출력의 '환각 (Hallucination)'이란 무엇이며, 이를 줄이기 위해 어떤 평가 지표를 설계할 수 있을까요?
2.  자동화된 LLM 출력 평가 지표만으로 충분하지 않은 이유는 무엇이며, '인간 검증 (Human-in-the-Loop)'이 필요한 주요 상황은 언제일까요?
3.  지속적 평가 (Continuous Evaluation)가 LLM 기반 애플리케이션에서 중요한 이유는 무엇이며, MLOps 파이프라인에 어떻게 통합할 수 있을까요?
4.  RAG 시스템의 출력 품질을 평가할 때, 일반적인 텍스트 생성 LLM 평가와는 어떤 차이점이 있을까요?

**동료에게 설명한다면?**
LLM 출력 검증 품질 지표를 설계하는 과정과 그 중요성을 기술 배경이 다른 동료에게 어떻게 설명하여 이해를 도울 수 있을까요?

**실습 과제**
당신이 개발하고 있는 LLM 기반 챗봇이 사용자 질문에 대한 답변을 생성합니다. '사용자 만족도'를 측정하기 위한 구체적인 정량적 평가 지표 3가지와, 각각의 지표를 측정하기 위한 방법을 설계하고 간단한 예시 코드를 구상해보세요. (예: 긍정/부정 감성 분석, 답변 길이, 핵심 키워드 포함 여부 등) 예를 들어, `evaluate_user_satisfaction` 함수를 만들어 보세요.
