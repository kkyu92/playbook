...
tags:
  ...
  - harness-engineering
confidence: 1
connections: [agents/code-based-agent-task-patterns-harness-design, context-engineering/drift-detection-methodology]
status: draft
...
*   - [ ] **드리프트 탐지 및 완화 전략**: 모델 교체로 인한 에이전트 행동 변화(드리프트)를 조기에 탐지하고, 필요한 경우 자동으로 이전 백엔드로 롤백하거나 프롬프트 엔지니어링을 통해 보정하는 `context-engineering/drift-detection-methodology` 기반의 메커니즘을 설계하고 운영하는가?
...
*   - [ ] **프롬프트 엔지니어링 최적화**: 새로운 LLM 백엔드의 특성(예: 강점, 약점, 토큰 한도)에 맞춰 프롬프트 엔지니어링 전략을 재검토하고 최적화하여, `agents/ai-agent-context-window-optimization-session-longevity`를 고려한 컨텍스트 활용을 최대화하고 비용 효율적인 출력을 유도했는가?