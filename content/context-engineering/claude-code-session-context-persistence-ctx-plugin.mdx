...
tags:
  ...
  - harness-engineering
confidence: 1
connections: [context-engineering/ambient-knowledge-injection, agents/enterprise-agent-management-patterns-llm-orchestration, agents/llm-agent-goal-achievement-planning-patterns, context-engineering/claude-code-session-web-mobile-monitoring-implementation, infrastructure/sentry-pii-scrubbing-beforesend, evaluation/llm-output-validation-quality-metrics-design, context-engineering/claude-code-ctx-plugin-context-persistence, prompt-engineering/on-device-llm-inference-efficiency-patterns, context-engineering/claude-code-harness-large-codebase-architecture-patterns, agents/ai-agent-persistent-memory-automated-context-management, infrastructure/claude-cli-persistent-multi-session-web-terminal]
status: draft
...
*   [ ] **사용자 피드백 루프**: 개발자가 에이전트가 주입된 컨텍스트를 어떻게 활용하고 있는지 시각적으로 확인하고, 필요한 경우 컨텍스트를 수동으로 수정하거나 재정의할 수 있는 UI/UX 요소가 제공되는가? (`context-engineering/claude-code-session-web-mobile-monitoring-implementation` 고려)