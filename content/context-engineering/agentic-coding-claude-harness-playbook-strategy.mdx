...
tags:
  ...
  - harness-engineering
confidence: 1
connections: [project-ops/claude-goal-playbook-worker-automation, project-ops/claude-code-harness-skillset-reinforcement-agent-playbook, infrastructure/claude-code-ctx-plugin-context-persistence, context-engineering/claude-code-harness-large-codebase-architecture-patterns, evaluation/claude-code-generation-validation-pipeline-extension, context-engineering/zerostack-rust-agent-harness-extension, agents/qwen3-7-max-agent-model-evaluation-integration, infrastructure/claude-code-harness-cost-optimization-reassessment, context-engineering/flue-framework-integration-claude-code-harness-reconfigurati, infrastructure/claude-cli-persistent-multi-session-web-terminal, infrastructure/claude-max-subscription-github-actions-self-hosted-runner-pattern, harness-engineering/glm-5-3-code-generation-agent-harness-enhancement, harness-engineering/forge-harness-ai-code-verification-gate-strategy, harness-engineering/claude-commerce-agent-blueprint-harness-extension, prompt-engineering/agent-message-analysis-pipeline-llm-optimization]
status: draft
...
2.  **컨텍스트 관리(Context Management)**: 에이전트 간의 효율적인 협업을 위해서는 이전 작업의 컨텍스트를 정확하게 전달하고 관리하는 메커니즘이 필수적입니다. `infrastructure/claude-code-ctx-plugin-context-persistence`와 같은 접근 방식은 LLM의 컨텍스트 윈도우 한계를 극복하고 장기 세션을 유지하는 데 중요합니다.
...
*   **언제 나쁜가**: 에이전트가 의도치 않은 방향으로 행동(드리프트)하거나, 예측 불가능한 결과를 도출할 수 있습니다. 특히 복잡한 상황에서 미묘한 오류를 발생시킬 경우, 이를 감지하고 수정하는 데 상당한 노력이 필요할 수 있으며, 이는 `context-engineering/silent-drift-family-pattern`과 같은 문제를 야기할 수 있습니다.
...
- [ ] 에이전트 간의 **컨텍스트 전달 메커니즘**이 견고하게 작동하며, 정보 손실이나 오염 없이 다음 단계로 정확하게 전달되는가? (`infrastructure/claude-code-ctx-plugin-context-persistence` 패턴 적용 확인)
- [ ] 에이전트의 작업 결과에 대한 **자동화된 검증(테스트, 린트, 스캔 등) 시스템**이 구축되어 있으며, 실패 시 적절한 피드백을 제공하는가?
- [ ] 비정상적인 에이전트 행동(드리프트)을 **감지하고 알림을 발생시키는 모니터링 시스템**이 있는가? (`context-engineering/drift-detection-methodology` 적용)
- [ ] LLM API 호출 **비용과 성능 지표(latency, token usage)**를 지속적으로 추적하고 최적화 계획이 수립되어 있는가?
- [ ] 사용자 또는 허브 에이전트가 **에이전트의 진행 상황을 투명하게 확인**하고, 필요 시 개입하거나 중단할 수 있는 인터페이스를 제공하는가? (`context-engineering/claude-code-session-web-mobile-monitoring-implementation`와 같은 대시보드 존재 여부)
