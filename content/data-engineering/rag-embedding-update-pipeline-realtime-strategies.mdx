...
tags:
  ...
  - data-engineering
confidence: 1
connections: [data-engineering/llm-data-acquisition-high-quality-datasets, data-engineering/llm-fine-tuning-high-quality-data-preprocessing-refinement, prompt-engineering/rag-prompt-optimization-retrieval-augmented-generation, data-engineering/vector-db-embedding-data-optimization-patterns, data-engineering/rag-search-performance-evaluation-metrics-design, evaluation/llm-input-output-data-pipeline-design-patterns, evaluation/rag-application-evaluation-retrieval-generation-quality, data-engineering/llm-fine-tuning-high-quality-data-construction-management, frontend-ai/kuku-concepts-playbook-local-markdown-enhancement, project-ops/ai-model-serving-harness-enterprise-deployment-management, agents/mirage-ai-agent-unified-filesystem-integration, data-engineering/rag-data-preprocessing-embedding-optimization-strategies, data-engineering/vector-database-optimization-llm-rag-performance, context-engineering/on-device-vector-db-jit-semantic-search, data-engineering/on-device-vector-db-jit-semantic-search-lsm-hnsw]
status: draft
...
*   **적응형 임베딩 모델 (Adaptive Embedding Models)**: 특정 도메인이나 기업 데이터에 특화된 경량화된 임베딩 모델을 온프레미스 또는 엣지 환경에서 실시간으로 업데이트 및 배포하여, 일반적인 모델 대비 높은 관련성과 성능을 제공합니다. 이는 `context-engineering/drift-detection-methodology`와 연관되어 임베딩 모델의 성능 저하를 감지하고 자동으로 재학습하는 파이프라인을 포함할 수 있습니다.
