...
tags:
  ...
  - prompt-engineering
confidence: 1
connections: [context-engineering/ambient-knowledge-injection, evaluation/llm-response-quality-automated-evaluation-metrics-design-str, prompt-engineering/llm-interactive-ui-frontend-implementation, agents/gpt-5-5-agent-model-evaluation-integration-strategies, agents/multi-persona-llm-output-validation-enhancement, infrastructure/guard-test-pattern, agents/llm-agent-persistent-goal-management-long-term-tasks]
status: draft
...
*   **가드레일 (Guardrails)**: LLM이 부적절하거나 안전하지 않은 내용을 생성하지 않도록 특정 규칙을 적용한다. `infrastructure/guard-test-pattern`과 같은 접근 방식이 여기에 해당한다.
