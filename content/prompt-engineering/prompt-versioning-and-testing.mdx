...
1.  LLM의 비결정성 및 프롬프트의 민감성이 프롬프트 버전 관리와 테스트를 왜 필수적으로 만드는지 설명해 보세요.
2.  프롬프트 테스트를 위해 골든 데이터셋을 구축하는 것이 왜 중요한지, 그리고 어떤 정보를 포함해야 하는지 이야기해 보세요.
3.  CI/CD 파이프라인에 프롬프트 테스트를 통합했을 때, iOS/프론트엔드 애플리케이션 개발 관점에서 얻을 수 있는 주요 이점은 무엇인가요?
4.  2026년 트렌드 중 "LLM-as-a-Judge 기반 자동 평가" 방식이 프롬프트 테스트에 어떤 혁신을 가져올 수 있을지 본인의 생각을 말해 보세요.
5.  이 글에서 제시된 Python 기반의 프롬프트 테스트 코드 예제를 보고, Swift나 TypeScript 환경에서 유사한 테스트를 구현한다면 어떤 도구와 방식으로 접근할지 구상해 보세요.

**이 개념을 동료에게 설명한다면, 가장 먼저 어떤 부분부터 강조하며 이해를 도울 건가요?**

**실습 과제:**

당신이 개발하고 있는 iOS/프론트엔드 애플리케이션에 사용자 질문을 기반으로 제품 정보를 제공하는 AI 비서 기능이 추가될 예정입니다. 이 기능을 위한 초기 `system` 프롬프트와 `user` 프롬프트 템플릿을 각각 JSON 파일로 작성하고, 최소 3개 이상의 테스트 케이스를 포함하는 Python `pytest` 기반의 테스트 스크립트를 작성하여 프롬프트가 의도된 답변 형식(예: 300자 이내, 핵심 키워드 포함, 한국어 답변)을 준수하는지 검증해 보세요. 골든 데이터셋은 직접 간단하게 만들어 사용해도 좋습니다.