...
tags:
  ...
  - prompt-engineering
confidence: 1
connections: [prompt-engineering/prompt-versioning-and-testing, evaluation/llm-output-validation-quality-metrics-design, harness-engineering/guard-test-pattern, harness-engineering/drift-detection-methodology, context-engineering/ambient-knowledge-injection, journal/playbook-journal-003-drift-silent-bugs, harness-engineering/vercel-breach-third-party-ai-oauth-supply-chain-security, agents/qwen3-6-max-preview-llm-agentic-coding-evaluation, agents/kimi-k2-6-open-source-agentic-coding-exploration]
status: draft
...
## 자기 점검 (Self-Assessment)

1.  LLM 시스템에서 역할 정의(Role Definition)가 중요한 세 가지 이유는 무엇인가요?
2.  제약 조건 설계(Constraint Design) 시 고려해야 할 주요 유형 3가지를 설명하고 각 유형의 필요성을 예시와 함께 제시해 보세요.
3.  2026년 LLM 프롬프트 최적화 트렌드 중 두 가지를 설명하고, 이것이 기존의 정적 프롬프팅과 어떻게 다른지 설명해 보세요.
4.  만약 동료 개발자에게 LLM 프롬프트 최적화의 핵심 전략인 역할 정의와 제약 조건 설계에 대해 설명해야 한다면, 어떤 비유나 핵심 메시지를 사용하여 이해를 도울 건가요?
5.  **실습 과제**: 특정 웹사이트의 상품 리뷰를 요약하는 LLM 시스템을 설계한다고 가정해 봅시다. 이 시스템의 프롬프트에 적용할 페르소나, 대상 독자, 목표를 정의하고, 최소 3가지 이상의 제약 조건(형식, 길이, 어조 등)을 구체적으로 설계해 보세요. 그리고 이를 바탕으로 실제 파이썬 코드를 이용해 프롬프트 문자열을 구성해 보세요.