...
tags:
  ...
  - harness-engineering
confidence: 1
connections: [harness-engineering/guard-test-pattern, harness-engineering/drift-detection-methodology, infrastructure/sentry-pii-scrubbing-beforesend, journal/playbook-journal-007-sentry-sensitive-fields-beforesend, prompt-engineering/llm-prompt-optimization-role-definition-constraint-design, harness-engineering/question-own-defaults, context-engineering/ambient-knowledge-injection, frontend-ai/frontend-ai-model-optimization-web-deployment]
status: draft
...
1.  Vercel 침해와 같은 서드파티 공급망 공격을 방어하기 위해 OAuth 통합 시 최소 3가지 이상의 보안 강화 전략을 설명하세요.
2.  AI 서비스 통합 시 데이터 프라이버시, 모델 무결성, API 키 관리 외에 2026년에 더욱 중요해질 것으로 예상되는 보안 고려사항은 무엇이며 그 이유는 무엇입니까?
3.  `Refresh Token Rotation`의 개념과 왜 이것이 `Long-lived Refresh Token` 유출 위험을 줄이는 데 중요한지 설명하세요.
4.  `Prompt Injection` 공격은 무엇이며, 이를 방어하기 위한 주요 기술적 접근 방식 두 가지는 무엇입니까?

**동료에게 설명한다면?**

Vercel 침해 사례를 바탕으로, 우리 회사의 서드파티 AI 및 OAuth 통합 보안 가이드라인을 동료에게 설명한다고 가정해봅시다. 핵심적인 문제점과 우리가 취해야 할 구체적인 조치들을 어떻게 전달할 것인가요?

**실습 과제:**

새로운 AI 기반 이미지 생성 서드파티 서비스 (`ImageGenAI`)와 우리 애플리케이션을 통합해야 한다고 가정합시다. 이 서비스가 `OAuth`를 통해 사용자 인증을 받고, 생성된 이미지를 저장하기 위해 `AWS S3` 버킷에 접근해야 합니다. 이 통합을 위한 보안 점검 체크리스트를 최소 5가지 항목으로 구성하고, 각 항목에 대한 구체적인 보안 요구사항을 명시하세요. 특히 `ImageGenAI`가 요청할 수 있는 `OAuth Scope`와 `S3 접근 권한`에 대해 최소 권한 원칙을 어떻게 적용할지 포함해야 합니다.