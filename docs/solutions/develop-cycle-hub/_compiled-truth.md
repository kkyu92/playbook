...
- **코드 게이트 승격**: 미착수 — 개별 패턴이 서로 다른 sub-system (fake batch / orphan slug / INDEX.md rebase)
...
| 2 | connections slug `<category>/<slug>` 접두사 누락 → orphan | MDX connections 작성 시 `<category>/` prefix 필수 체크 | 2nd 재발 (cycle 1083 4건 수정) |
